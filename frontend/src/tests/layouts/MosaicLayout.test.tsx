...
import { MosaicLayout } from '../../layouts/mosaic/MosaicLayout.js'
...
describe('MosaicLayout', () => {
  it('renders without crashing', () => {
    const { container } = render(<MosaicLayout {...mockProps} />)
    expect(container.firstChild).toBeTruthy()
  })

  it('renders the tessellation canvas behind the widgets', () => {
    const { container } = render(<MosaicLayout {...mockProps} />)
    expect(container.querySelector('canvas')).toBeTruthy()
  })

  it('shows the clock time', () => {
    const { getByText } = render(<MosaicLayout {...mockProps} />)
  ...
  it('shows the pihole ripple caption', () => {
    const { getByText } = render(<MosaicLayout {...mockProps} />)
    expect(getByText(/50,000 queries · 15% blocked/)).toBeTruthy()
  })
  ...
  it('shows now playing when a session is active', () => {
    ...
    const { getByText } = render(<MosaicLayout {...mockProps} sessions={[session]} />)
