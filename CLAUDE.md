...
│   │   ├── layouts/          # Theme layouts (zen, classic, terminal, newspaper, departures, fridge, observatory, flux, mosaic, fractal)
...
│   │   │   └── fractal/      # Endless self-similar canvas zoom, randomly generated (day/night theming)
...
- **Day/night theming** — `useDayNight` hook applies `html.dark`/`html.light` CSS classes; Zen, Newspaper, Observatory, Flux, Mosaic, and Fractal respond to them (shared `useIsDark` hook in `hooks/useIsDark.ts`)
- **Kinetic canvas themes (Flux, Mosaic, Fractal)** — `FluxField` (particle flow field), `MosaicField` (hex tessellation + data ripples), and `FractalField` (endless self-similar zoom; per-octave motifs seeded by depth + a per-load salt) each drive a single imperative `requestAnimationFrame` loop reading live data from refs (no per-frame React renders); all FPS-capped with count/layer tunables at the top of the file for low-power hardware. Shared floating widgets (clock, sober chip, glass cards, now playing) live in `components/KineticOverlay.tsx`
