...
- **Home Assistant** — `GET /api/homeassistant` proxies HA's `/api/states` (Bearer token auth) and curates it into a `HomeAssistantSummary`. Read-only — no service calls. Returns `configured: false` (HTTP 200) when env vars are missing so layouts can render a setup note. **No layout renders the device schedule any more** — it was replaced by the Apple Health panel (a list of which lights are on tells you nothing you can't see by looking up). What's still rendered is `temps` (thermal section), the two humidity readouts in that section's legend, and the `reachable` flag in the footer's telemetry line. The device/sensor curation below still runs in `buildSummary` and is kept because it's the expensive part to rebuild and costs nothing extra — the `/api/states` call is needed for the temperatures regardless:
  ...
  - **Readouts** — `sensors` is a fixed set (interior/exterior temperature, then each one's humidity, paired by `…_temperature` → `…_humidity`) relabelled Interior/Exterior rather than showing HA's long entity names. Temperature and humidity round to whole units. The Blueprint thermal section reads the two `… RH` entries by that label
...
- **Thermal history** — the same endpoint also returns `temps`: 24 h of indoor vs outdoor temperature from HA's `/api/history/period`, resampled into 48 half-hour buckets (cached 5 min separately from the 20 s states cache, since history is the expensive call). Sensors are auto-detected from `device_class: temperature` entities by name (outdoor/patio/… vs indoor/living/…) and can be pinned with `HOMEASSISTANT_{INDOOR,OUTDOOR}_TEMP_ENTITY`. HA only records on change, so buckets forward-fill the last known reading. A history failure degrades to `available: false` without blanking the rest of the payload. Drawn by `layouts/blueprint/ThermalProfile.tsx`
- **Apple Health** — `GET /api/fitness` reads the `healthkit` service on popos (LAN address, read-only API key) and curates it into a `FitnessSummary`. One upstream `/api/summary` call plus six cheap ones (medications, workouts, and the blood-pressure / weight / sleep / dietary-energy / step series), all behind a 5 min cache with a last-good fallback; the trend calls are individually optional, so a failing series costs one plot rather than the panel. Rendered by the Blueprint layout's "Occupant · Health Schedule" (`layouts/blueprint/HealthSchedule.tsx` + `HealthPlots.tsx`). Things worth knowing:
  - **Three distinct empty states, deliberately not collapsed** — unconfigured (setup note), nothing ever logged for a metric (a dash or an omitted plot), and a phone that has stopped syncing (`ingest.ageHours` > 26 → a staleness line above figures that are real but old)
  - **A held reading is not a zero** — when a metric's units change upstream (lb → kg), the health service withholds the value until a human acknowledges it, returning a slot with `heldFor`. Those render as "HELD"/"UNITS CHANGED — SERIES HELD" and are listed in `heldMetrics`. Rendering `0` there is exactly the silent-wrong-number the hold exists to prevent
  - **Day boundaries come from upstream** — every `ts` is UTC but "today" is the health service's `localDate` in its own `timezone`; `localDateOf()` converts with `Intl` so the kiosk and the server can't disagree about what day it is. Medication and workout stamps are Health Auto Export's local wall-clock strings, so their date is read off the string directly
  - **Goals** — `WORKOUT_SPECS` in `fitnessService.ts` holds one entry per discipline (cycling: 5 days × 60 min; lifting: 3 days, any length), each matched against the workout's name and Apple `activityType`, each against a rolling 7-day window — so "ride today" means the same thing on a Monday as on a Friday. A day counts at half the target length, or at any length when `targetMinutes` is 0. Days and minutes are env-tunable (`FITNESS_*`), as is the flat calorie budget. The medication streak survives a dose that simply isn't due yet: it counts back from yesterday when today is unlogged
  - **Sleep score** — Apple has no such number, so `scoreNight()` computes one: duration vs 8 h (60%), deep+REM share vs 35% (25%), efficiency vs 90% (15%), each capped at its target. `totalsleep` is the field that carries the night — `asleep` is 0 on Apple Watch exports. The headline score and hours average cover the **recorded nights in the 7 calendar days ending today** — the same week the plot draws. A night with no record (or 0 h) is unmeasured, not zero: it shows as an empty slot and is left out of both averages
...
- `HEALTHKIT_URL`, `HEALTHKIT_READ_KEY` — Apple Health via the `healthkit` service on popos (LAN address; the **read** key, never the ingest key)
- `FITNESS_CALORIE_BUDGET`, `FITNESS_WORKOUT_DAYS`, `FITNESS_WORKOUT_MINUTES`, `FITNESS_LIFT_DAYS` — optional goals (default 1900 kcal; cycling 5 days/week × 60 min; lifting 3 days/week, any length)
